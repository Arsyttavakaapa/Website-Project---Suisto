...
$sql='select *, DAYNAME(events_date) as paiva, time_format(events_time, "%H:%i") as time, date_format(events_date, "%e.%c") as date from events left outer join tiketti on events.id=tiketti.events_id where month(events_date)=? and year(events_date)=? order by events_date';
...
//suoritamme komennon
mysqli_stmt_bind_result($stmt, $id, $event_name, $event_date, $event_time, $description, $created, $updated, $tiketti_id, $events_id, $link, $paiva, $time, $date);
